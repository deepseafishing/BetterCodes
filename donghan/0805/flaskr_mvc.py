...
def sorting():
    if dataStorage.sort_key == "Descend":
        new_list=[]
        max_dict={}
...
def del_entry(idx):
    ...
    '''for i in range(len(dataStorage.database)): # 'data' is copy of 'for sentence'
...
def sort_entry():
    if dataStorage.sort_key == "Descend":
        dataStorage.sort_key = "AAscend"

    else:
        dataStorage.sort_key = "Descend"
    return redirect(url_for('show_entries'))
