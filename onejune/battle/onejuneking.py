...
# the 4 functions
	...
	def firstHit():
		...
		if board[x+1][y] == Board.Status.EMPTY:
			x = x+1
			direction = ['right', 'horizon']
			return x, y
		elif board[x-1][y] == Board.Status.EMPTY:
			x = x-1
			direction = ['left', 'horizon']			
			return x, y
		elif board[x][y+1] == Board.Status.EMPTY:
			y = y+1
	...
	def hitHard():
		...
		if turn_up == True && log['result'] == Record.Status.MISSED:
			...
			if direction[0] == 'right':
				if record.get_status_at(x+1, y) == Record.Status.EMPTY:
					x = x+1
					return x, y
				else:
					direction[0] = 'left'
				...
				if record.get_status_at(x, y+1) == Record.Status.EMPTY:
					y = y+1
					return x, y
				else:
					direction[0] = 'down'
